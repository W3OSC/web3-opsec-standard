...
**SPF record too complex:** SPF has a 10 DNS lookup limit. Don't include too many services. Each `include:` counts as a lookup.

**Jumping straight to DMARC `p=reject`:** Always start with `p=none` to monitor. Fix any SPF/DKIM issues before enforcing.

**Enabling MTA-STS enforce mode immediately:** Start with `testing` mode. Review reports. Fix issues. Then switch to `enforce`.

**DNSSEC without monitoring:** Keys expire. If you don't monitor and rotate them, your domain stops resolving entirely.

**Adding TLSA without DNSSEC:** TLSA requires DNSSEC. If DNSSEC isn't working, TLSA makes things worse.

**Forgetting CAA:** Takes 5 minutes to add and prevents unauthorized certificate issuance. There's no reason not to have it.
...
- DMARC Analyzer: Test and monitor DMARC reports
...
**Do first:** SPF, DKIM, DMARC (with gradual enforcement), CAA  
**Do next:** MTA-STS  
**Do if high-risk:** DNSSEC with proper operations  
**Do if required:** TLSA (only after DNSSEC is mature)

Start with email security. It's the highest ROI and easiest to implement. Add CAA immediately—it takes 5 minutes. Only implement DNSSEC if you have operational capacity for ongoing key management. TLSA is optional unless required by regulation or you're managing critical infrastructure.
